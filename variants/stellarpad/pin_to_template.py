...
pins_energia = open("pins_attoduino.h","r").readlines()

pins = []
indeclares = False
for l in pins_energia:
    if l.count("attoduino pin definition begin") == 1:
        indeclares = True
        continue
    if l.count("attoduino pin definition end") == 1:
        break
...
# pin 0 is actually used, so don't start with dummy values
...
for pin in pins:
    ...
    #boardlabel    = "/* not labeled */"

    # make sure the pins are progressing sequentially
