...
#ifdef ATTODUINO
  qualityFactor = fabsf(qualityFactor);             // cannot be negative
#else
  qualityFactor = abs(qualityFactor);             // cannot be negative
#endif
...
void FilterTwoPole::setAsFilter( OSCILLATOR_TYPE ft, float frequency3db, float initialValue ) {
  ...
  if( ft == TWO_POLE_HIGHPASS )     IsHighpass = true;
  else                              IsHighpass = false;

  X = initialValue;
  // set as a bessel filter
...
float FilterTwoPole::input( float drive ) {
  float Fprev = drive;                      // needed when using filter as a highpass
  ...
  X += .5*(V+Vprev) * dt;                         // step position, using average V to reduce error
...
float FilterTwoPole::output() {
  if( IsHighpass )
  return Fprev-X;    // the difference of the recent input, and the filtered value
  else
